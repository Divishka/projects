...
int	ft_output_err(int code, char *str1, char *str2, char *str3)
{
	ft_putstr_fd(ERROR_PREFIX, 2);
	if (str1 != NULL)
...
int	ft_command_err_slesh(char *name_f)
{
	...
	if (opendir(name_f) != NULL)
		return (ft_output_err(1, name_f, ": Is a directory\n", NULL));
	else
	{
		if (stat(name_f, &buff) >= 0)
			return (ft_output_err(126, name_f,
					": Permission denied\n", NULL));
		if (stat(name_f, &buff) >= 0)
			return (ft_output_err(127, name_f,
					": Not a directory\n", NULL));
		return (ft_output_err(127, name_f,
...
int	ft_command_err(char *name_f)
{
	...
	if (name_f[0] == '.' && name_f[1] == '\0')
		return (ft_output_err(1, name_f,
				": filename argument required\n",
				".: usage: . filename [arguments]\n"));
	if (name_f[0] == '.' && name_f[1] == '.' && name_f[2] == '\0')
		return (ft_output_err(127, name_f, ": command not found\n", NULL));
